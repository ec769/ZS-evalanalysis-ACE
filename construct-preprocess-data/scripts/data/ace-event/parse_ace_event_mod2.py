...
class Entity(TokSpan):
    ...
    def to_json(self):
        return [*self.adjusted_span_doc, self.mention_type, str(self.text_string)]
...
class Event:
    ...
    def to_json(self):
        ...
        for arg in self.arguments:
            ...
            args.append([*arg.adjusted_span_doc, arg_role,str(arg.text_string)])
    ...
    def _populate_entity_list(self):
        ...
        for one_entity in xml_root[0].findall('entity'):
            ...
            for one_entity_mention in one_entity.findall('entity_mention'):
                ...
                start_char_h, end_char_h, text_string_h = self._get_chars(tentative_start, tentative_end)
                field_to_find = "extent"
                tentative_start = int(one_entity_mention.find(field_to_find)[0].attrib['START'])
                tentative_end = int(one_entity_mention.find(field_to_find)[0].attrib['END'])

                start_char_e, end_char_e, text_string_e = self._get_chars(tentative_start, tentative_end)

                # Parser chokes on the space.
                if (self._doc_key == "soc.history.war.world-war-ii_20050127.2403" and
                    (text_string_h.text == "lesliemills2002@netscape. net" or text_string_e.text == "lesliemills2002@netscape. net")):
                    continue

                # Keep option to ignore pronouns.
                flavor = "pronoun" if one_entity_mention.attrib["TYPE"] == "PRO" else "entity"

                entry = Entity(start_char_h, end_char_h, str(text_string_h)+"...."+str(text_string_e), mention_id=mention_id,
            ...
            for one_event_mention in one_event.findall('event_mention'):
                ...
                for one_event_mention_argument in one_event_mention.findall('event_mention_argument'):
                    ...
                    if self._real_entities_only and this_entity.flavor not in self._allowed_flavors:
                        continue
                    start_char_h, end_char_h, text_string_h = (this_entity.start_char,
                    ...
                    start_char_e, end_char_e, text_string_e = self._get_chars(
                    ...
                    to_append = EventArgument(start_char_h, end_char_h, str(text_string_h), argument_id,
                                              argument_role)
        ...
        if self._include_entity_coreference:
            ...
            for entity_id, mention_ids in self.entity_mention_clusters.items():
                ...
                for mention_id in mention_ids:
                    ...
                    #print(mention_json)
                    cur_cluster.append(mention_json)
...
def one_fold(fold, output_dir, heads_only=True, real_entities_only=True, include_pronouns=False,
    ...
    doc_path = "./construct-preprocess-data/data/ace-event/raw-data"
    split_path = "./construct-preprocess-data/scripts/data/ace-event/event-split"
